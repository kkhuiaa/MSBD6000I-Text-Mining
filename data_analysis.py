...
news_df_merged = news_df.merge(bert_pca_df, left_index=True, right_index=True, how='left')
...
X_train = price_700_df_merged.drop(price_col_list+['date'], axis=1).iloc[:4*data_len//5:, :]
X_test = price_700_df_merged.drop(price_col_list+['date'], axis=1).iloc[4*data_len//5:, :]
y_train = price_700_df_merged['Open(t+1) >= Close'].iloc[:4*data_len//5]
y_test = price_700_df_merged['Open(t+1) >= Close'].iloc[4*data_len//5:]
...
random_state_model = 1
n_jobs = 15
xgbc = XGBRegressor(random_state=random_state_model, n_jobs=n_jobs, eval_set=[(X_train, y_train)], eval_metric='auc', early_stopping_rounds=5)
param_dist = {'n_estimator': randint(10, 15)
            , 'max_depth': randint(1, 3)
            , 'learning_rate': uniform(.005, .01)
            , 'min_child_weight': uniform(.1, .015)
...
#%%
lag = 50
X_lstm = price_700_df_merged.drop(shifting_columns+['Open(t+1) >= Close', 'Open(t+1)', 'date'], axis=1).copy()
input_dim, input_col = X_lstm.shape[0], X_train_lstm.shape[1]
...
X_lstm_train, X_lstm_test, y_lstm_train, y_lstm_test = train_test_split(np.array(X_lstm_list)
    , np.array(y_lstm_list), test_size=.2, random_state=1, stratify=np.array(y_lstm_list))
...
lstm_model.add(LSTM(units=32, dropout=.5, recurrent_dropout=.5))
...
lstm_model.fit(X_lstm_train, y_lstm_train, epochs=200, verbose=2, validation_data=[X_lstm_test, y_lstm_test], callbacks=[early_stopping], batch_size=256)
...
for i in range(lag, len(X_lstm)):
    X_lstm_list.append(X_lstm[i-lag:i, :])
    y_lstm_list.append(np.array(price_700_df_merged['Open(t+1)'].iloc[i]))

X_lstm_train, X_lstm_test, y_lstm_train, y_lstm_test = train_test_split(np.array(X_lstm_list)
    , np.array(y_lstm_list), test_size=.2, random_state=1)
print('X lstm train shape', X_lstm_train.shape)
print('y lstm train shape', y_lstm_train.shape)
...
print('r2 in train:', r2_score(y_lstm_train, lstm_model.predict(X_lstm_train)))
print('r2 in test:', r2_score(y_lstm_test, lstm_model.predict(X_lstm_test)))

# %%
price_700_df_merged['Open(t+1)']
